...
use Horde_Kolab_Storage_Cached;
...
use Horde_Kolab_Storage_List_Base;
use Horde_Kolab_Storage_List_Query_List_Defaults_Bail;
use Horde_Kolab_Storage_QuerySet_Cached;
...
use Horde\Kolab\Storage\Test\Stub\HistoryPrefix;
...
extends PhpUnitTestCase
{
    ...
    protected function createStorage(
        ...
        return new Horde_Kolab_Storage_Uncached(
            ...
            $factory,
            $this->getMockBuilder(Horde_Kolab_Storage_Cache::class, array(), array(), '', false, false)->disableOriginalConstructor()->getMock(),
        ...
        $factory = new Horde_Kolab_Storage_Factory(
            array_merge(
                array(
                    'driver' => 'mock',
                    'params' => $data,
                    'logger' => $this->createMock(Horde_Log_Logger::class),
                    'history_prefix_generator' =>  new HistoryPrefix()
                ),
                $params
            )
        );
    ...
    {
        $factory = $this->completeFactory($factory);
        $this->mockDriver = $this->createMock(Horde_Kolab_Storage_Driver::class);
        return new Horde_Kolab_Storage_List_Base(
            $this->mockDriver,
            new Horde_Kolab_Storage_Factory()
        );
